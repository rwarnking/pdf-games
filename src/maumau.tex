...
function initialiseButtons() {
    ...
    var cb = this.getField("cmCards");
    cb.checkThisBox(1);
    for (var i = 0; i < 5; i++) {
        cardsPerPlayer += cb.isBoxChecked(i) * (4 + i);
    };
...
function specialRules(cardIdx) {
    ...
    % Next player is not allowed to play
        ...
        curPlayer = (curPlayer + 1) \% playerCount;
        printInfoText("Player " + (curPlayer + 1) + " misses his turn.");
    }
    % TODO
    % Player may choose a color
    else if (c.charAt(0) == "B") {
        printInfoText("Player " + (curPlayer + 1) + " may choose a color.");
    }
    % Player needs to play another card
    else if (c.charAt(0) == "A") {
        curPlayer--;
        printInfoText("Player " + (curPlayer + 2) + " must play another card.");
    }
}
...
\begin{document}
    ...
    \subsection*{Rules}
        ...
        \ChoiceMenu[name=cmCards, bordercolor=gray, radiosymbol=5, radio]{Card count per player: }
            {4, 5, 6, 7, 8}
        ...
        \begin{center}
            \begin{tabularx}{\textwidth}{@{} X X @{}}
                ...
                \CheckBox[name=cbAskip, bordercolor=gray, checkboxsymbol=5]{Ace - skip next player}
                %
            \end{tabularx}
        \end{center}
