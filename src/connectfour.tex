...
function addToColumn(x) {
    ...
    % Column is full
    if (y == -1) {
        printInfoText("This Column is full. Player " + (activePlayer + 1) + ": Select a valid column.");
        return;
    }
...
function checkGameover(x, y) {
    ...
    for (var dirIdx = 0; dirIdx < directions.length; dirIdx += 2) {
        ...
        for (var wp = -WINNING_PIECES + 1; wp < WINNING_PIECES; wp++) {
            ...
            if (nx < 0 || nx >= FIELD_WIDTH || ny < 0 || ny >= FIELD_HEIGHT || complField[ny][nx] != activePlayer + 1) {
                count = 0;
            } else {
                count++;
                maxCount = count;
