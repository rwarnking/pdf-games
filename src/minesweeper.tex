...
function fillMines() {
    ...
    mines = (BOARD_WIDTH * BOARD_HEIGHT) / 100 * this.getField("percentage").value;
    ...
    for (var i = 0; i < mines; i++) {
        x = getRandomInt(BOARD_WIDTH);
        y = getRandomInt(BOARD_HEIGHT);
        complBoard[y][x][1] = MINE;
    }
    ...
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
            ...
            if (complBoard[y][x][1] == MINE) {
                continue;
            }
            % Check the neighbourhood
            for (var xx = x - 1; xx <= x + 1; xx++) {
                for (var yy = y - 1; yy <= y + 1; yy++) {
                    if (xx < 0 || yy < 0 || xx >= BOARD_WIDTH ||
                        yy >= BOARD_HEIGHT || (xx == x && yy == y)
                    ) {
                        continue;
                    }
                    % Adjust count
                    if (complBoard[yy][xx][1] == MINE) {
                        complBoard[y][x][1]++;
                    }
                }
            }
...
function restart() {
    ...
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
             complBoard[y][x] = [HIDDEN, EMPTY, !LOCK];
        }
    }
...
function reveal(x, y) {
    ...
    if (gameMode == LOCK) {
        if (complBoard[y][x][2] != LOCK) {
            complBoard[y][x][2] = LOCK;
        } else {
            complBoard[y][x][2] = !LOCK
        }
        draw(x, y);
    ...
    if (complBoard[y][x][1] == EMPTY) {
        ...
        while (stack.length > 0) {
            ...
            for (var xx = elem[0] - 1; xx <= elem[0] + 1; xx++) {
                for (var yy = elem[1] - 1; yy <= elem[1] + 1; yy++) {
                    ...
                    if (xx < 0 || yy < 0 || xx >= BOARD_WIDTH || yy >= BOARD_HEIGHT ||
                        (xx == elem[0] && yy == elem[1]) || complBoard[yy][xx][0] == VISIBLE) {
                        continue;
                    }
                    % Reveal the cell
                    complBoard[yy][xx][0] = VISIBLE;
                    % Adjust counter for each cell
                    revealed++;
                    draw(xx, yy);
                    % If it is a number then do not add it to the stack
                    if (complBoard[yy][xx][1] != EMPTY) {
                        continue;
                    }
                    stack.push([xx, yy]);
        ...
        % Add one if there was one cell revealed
        revealed++;
    }
...
/**
 * Draw the cell at the coordinates x, y.
 *
 * @param {Number} x The x coordinate
 * @param {Number} y The y coordinate
 */
function draw(x, y) {
    ...
    if (complBoard[y][x][0] == VISIBLE) {
        var val = complBoard[y][x][1];
