...
const CREATURE_LIST = new Array(
    ["SHEEP", 0.9, T_FLUFF, [33, 35, 37, 43], 32,
    ...
    ["STONE", 2.3, T_STONE, [6, 13, 16, 39], 50,
    ...
    ["TREE", 1.5, T_GRASS, [11, 17, 20, 40], 39,
    ...
    ["SKELETOR", 1.2, T_GHOST, [4, 18, 31, 42], 40,
...
const ATTACK_LIST = new Array(
    % Water
    ["OverflowingLook", 1.0, T_WATER, 1], % 0 - DUCK
    ["BubbleKnock", 1.0, T_WATER, 1], % DUCK
    ["IcePrism", 1.0, T_WATER, 1],
    ["RunningWave", 1.0, T_WATER, 1],
    ["DrowningScreech", 1.0, T_WATER, 1], % SKELETOR
    ["Stormy Rush", 1.0, T_WATER, 1], % 5 - BIRD
    ["Flow Release", 1.0, T_WATER, 1], % STONE
    % Air
    ["Fearless Smoke", 1.0, T_AIR, 1],
    ["Dutiful Mist", 1.0, T_AIR, 1], % BIRD
    ["Wind Volley", 1.0, T_AIR, 1],
    ["Screaming Wind", 1.0, T_AIR, 1], % 10 - BIRD
    ["Stormy Axe", 1.0, T_AIR, 1], % TREE
    ["Typhoon Bullet", 1.0, T_AIR, 1],
    % Stone
    ["Meteor Fall", 1.0, T_STONE, 1], % STONE
    ["Earth Impact", 1.0, T_STONE, 1],
    ["Rockslide", 1.0, T_STONE, 1], % 15 -
    ["Gravity Blade", 1.0, T_STONE, 1], % STONE
    ["Tree Knock", 1.0, T_STONE, 1], % TREE
    ["Skeletal Scratch", 1.0, T_STONE, 1], % SKELETOR
    ["Cracking Peace", 1.0, T_STONE, 1],
    % % Grass
    ["Reducing Root", 1.0, T_GRASS, 1], % 20 - TREE
    ["Doubling Drain", 1.0, T_GRASS, 1],
    ["Arctic Branch", 1.0, T_GRASS, 1],
    ["Lotus Cannon", 1.0, T_GRASS, 1],
    ["Leafs Kiss", 1.0, T_GRASS, 1],
    ["Freezing Bark", 1.0, T_GRASS, 1], % 25 -
    % % Ghost
    ["Phantom Stomp", 1.0, T_GHOST, 1],
    ["Ghost Fist", 1.0, T_GHOST, 1],
    ["Spirit Blast", 1.0, T_GHOST, 1],
    ["Hungry Mind", 1.0, T_GHOST, 1],
    ["Vampiric Drain", 1.0, T_GHOST, 1], % 30 -
    ["Hells Ambush", 1.0, T_GHOST, 1], % SKELETOR
    ["Demonic Blood", 1.0, T_GHOST, 1],
    % % Fluff
    ["Breaking Cuddle", 1.0, T_FLUFF, 1], % SHEEP
    ["Sneaking Blow", 1.0, T_FLUFF, 1],
    ["Heavens Hit", 1.0, T_FLUFF, 1], % 35 - DUCK % SHEEP
    ["Summer Dance", 1.0, T_FLUFF, 1],
    ["Violence Void", 1.0, T_FLUFF, 1], % SHEEP
    ["Cloud Fall", 1.0, T_FLUFF, 1], % BIRD
    % % Fire
    ["Flaming Burst", 1.0, T_FIRE, 1], % STONE
    ["Venom Heat", 1.0, T_FIRE, 1], % 40 - TREE
    ["Poison Edge", 1.0, T_FIRE, 1],
    ["Hot Retribution", 1.0, T_FIRE, 1], % SKELETOR
    ["Radiating Charge", 1.0, T_FIRE, 1], % SHEEP
    ["Magic Teardrop", 1.0, T_FIRE, 1], % DUCK
    ["Boiling Rage", 1.0, T_FIRE, 1], % 45 -
    ["Forbidden Light", 1.0, T_FIRE, 1],
    ["Ash Whip", 1.0, T_FIRE, 1],
    ["Glowing Flash", 1.0, T_FIRE, 1],
    ["Nova Bullet", 1.0, T_FIRE, 1]
);
...
function inflictDMG(source, target, attack) {
    ...
    var dmg = Math.ceil(attack[DMG] * source[LVL] * creatureMul);
    target[CUR_HP] -= dmg;
    return dmg;
}
...
function updateButtontxt() {
    if (gamestate == MENU) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var dmgTxt = " (Does " + ATTACK_LIST[attacks[i]][DMG] + " type dmg per lvl)";
            this.getField(btn).value = ATTACK_LIST[attacks[i]][NAME] + dmgTxt;
        }
    } else if (gamestate == CHANGE) {
