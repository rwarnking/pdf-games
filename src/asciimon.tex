...
const FIELD_HEIGHT_H = (T - B) / 2.0; % TODO canvas half height
...
function setCanvasData() {
    ...
    canvas = {
        c: [
            [L_P + FIELD_WIDTH_H, B_P + FIELD_HEIGHT_H], % Player
            [L_E + FIELD_WIDTH_H, B_E + FIELD_HEIGHT_H] % Enemy
        ],
        l: [L_P, L_E],
...
function waterParticles(target) {
    ...
    particleSys.move = function(l, p) {
        ...
        const top = (bot + FIELD_HEIGHT_H * 2.0 * 0.75 - h);
...
function airParticles(target) {
    ...
    particleSys.emit = function(p) {
        ...
        if (p == leader) {
            ...
            yPos = bot + particles[p][HEIGHT] + getRandomInt((FIELD_HEIGHT_H - particles[p][HEIGHT]) * 2.0);
