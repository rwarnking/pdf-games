...
const ACTION_E_HP_UPDATE = 3;
const ACTION_E_CHOOSE = 4;
const ACTION_E_ATTACK = 5;
const ACTION_P_HP_UPDATE = 6;
const ACTION_P_CHANGE = 7;
const ACTION_END = 8;
...
var particleSys = { aliveParticles: 0 };
...
function restart() {
    ...
    if (animationProgress != FIELD_WIDTH || !particleSys.finished()) {
...
function attackEnemy(idx) {
    ...
    if (curEnemy[CUR_HP] == 0) {
        ...
        if (lvlup) {
            updatePlayerTxt();
            animatedString = curCreature[NAME] + " has slayn " + curEnemy[NAME] + " and leveled up. " +
                curEnemy[NAME] + " dropped " + itemName + ".";
        } else {
            animatedString = curCreature[NAME] + " has slayn " + curEnemy[NAME] +
                " and got " + exp + " experience. " + curEnemy[NAME] + " dropped " + itemName + ".";
        }
    } else {
...
function change(idx) {
    ...
    % TODO if enemy died before return ACTION_END
...
function itemSelect(idx) {
    ...
    if (item[NAME] == "Drink") {
        ...
        % Increase level by 2
        selCreature[CUR_EXP] = EXP_TABLE[selCreature[LVL] - MIN_LVL + 2];
...
function updateButtontxt() {
    if (guiState == MENU) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (name != "Empty") {
                name += " (" + ITEM_LIST[curPlayerItems[i][0]][DESC] + "), #" + curPlayerItems[i][1];
            }
            this.getField(btn).value = name;
        }
    }
...
function defaultParticleSys() {
    particles = Array.apply(null, Array(ATK_ANIM_CELLS)).map(_ => [
        ...
        0, 0, % SYMB, ROTATION
        2, [0, 0],  % MASS, MOMENTUM
        canvas.halfWidth / FIELD_HEIGHT, % WIDTH
        canvas.halfHeight / FIELD_HEIGHT, % HEIGHT
        (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 1.3 % SIZE
    ]);
...
function setCanvasData() {
    ...
    const PLAYER_BOT_RECT = this.getField("player" + (FIELD_HEIGHT - 1)).rect;
...
function airParticles(target) {
    ...
    particleSys.emit = function(p) {
        ...
        particles[p][WIDTH] = (canvas.halfWidth / FIELD_WIDTH) * 2.5;
...
function stoneParticles(target) {
    ...
    particleSys.emit = function(p) {
        % Create a particle with same width as height
        particles[p][WIDTH] = (canvas.halfHeight / FIELD_HEIGHT) * 2.0;
        particles[p][HEIGHT] = (canvas.halfHeight / FIELD_HEIGHT) * 2.0;
        particles[p][SIZE] = (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 0.9;
...
function standardParticles(target) {
    ...
    particleSys.emit = function(p) {
        particles[p][WIDTH] = (canvas.halfHeight / FIELD_HEIGHT) * 2.0;
        particles[p][HEIGHT] = (canvas.halfHeight / FIELD_HEIGHT) * 2.0;
        particles[p][SIZE] = (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 2.0;
 ...
 * @param {Number} p The particle index.
 */
function showAndUpdateParticleField(p) {
    var particle = this.getField("atkanim" + p);
...
/**
 * Adds the current creature of the given owner to the corresponding field.
 * The strings are simply concatenated line by line to the current field value.
...
\begin{document}
    ...
    \begin{Form}
        ...
        \columnbreak
            % Tooltip object
            \begin{flushright}%
                \TextField[backgroundcolor=white, bordercolor=white,
                    name=hintbox, readonly=true, hidden]{}
        ...
        \begin{multicols}{2}
            ...
            \begin{tcolorbox}%
                \TextField[name=enemyname, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}\newline

                \TextField[name=enemyhp, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}\newline

                \TextField[name=enemystatus, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}%
            \end{tcolorbox}%
        ...
        \columnbreak%
            ...
            \begin{tcolorbox}%
                \TextField[name=playername, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}\newline

                \TextField[name=playerhp, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}\newline

                \TextField[name=playerstatus, bordercolor=black, backgroundcolor=,
                    width=\linewidth, readonly=true]{}%
            \end{tcolorbox}%
        ...
        \begin{center}
            ...
            \begin{tabularx}{\textwidth}{@{} *{2}{X} @{}}%
                \PushButton[name=button1, bordercolor=white, borderwidth=0]{
                    \begin{tcolorbox}
                        \centering
                        \TextField[name=button1txt, bordercolor=, backgroundcolor=,
                            width=\linewidth, readonly=true]{}%
                        \strut
                    \end{tcolorbox}
                } &
                \PushButton[name=button2, bordercolor=white, borderwidth=0]{
                    \begin{tcolorbox}
                        \centering
                        \TextField[name=button2txt, bordercolor=, backgroundcolor=,
                            width=\linewidth, readonly=true]{}%
                        \strut
                    \end{tcolorbox}
                }\\
                \PushButton[name=button3, bordercolor=white, borderwidth=0]{
                    \begin{tcolorbox}
                        \centering
                        \TextField[name=button3txt, bordercolor=, backgroundcolor=,
                            width=\linewidth, readonly=true]{}%
                        \strut
                    \end{tcolorbox}
                } &
                \PushButton[name=button4, bordercolor=white, borderwidth=0]{
                    \begin{tcolorbox}
                        \centering
                        \TextField[name=button4txt, bordercolor=, backgroundcolor=,
                            width=\linewidth, readonly=true]{}%
                        \strut
                    \end{tcolorbox}
                }\\
        ...
        \begin{multicols}{2}
            ...
            \xintFor* #1 in {\xintSeq{0}{\atkAnimCells-1}} \do {%
                \TextField[%
                    ...
                    name=atkanim#1, readonly=true, borderwidth=0pt, hidden
                ]{}%
            }%
        ...
        \columnbreak
            ...
            \begin{tabularx}{\linewidth}{c | *{6}{@{\,\,} c @{\,\,} | } c}%
                  & Water & Air & Stone & Grass & Ghost & Fluff & Fire \\ \hline
