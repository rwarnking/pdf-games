...
var particles = Array.apply(null, Array(ATK_ANIM_CELLS)).map(_ => [DEAD, [0, 0], 0, 0, 2, [0, 0]]);

var particleSys = {
    iterationCount: ATK_DURATION - 1,
    iterationCur: 0,
    aliveParticles: 0,
    % TODO this is in one direction -> (FIELD_HEIGHT_H / FIELD_HEIGHT)
    particleRadius: FIELD_WIDTH_H / FIELD_HEIGHT,
...
function applyGravityToParticle(p) {
    const s = particleSys.particleRadius;
...
function waterParticles() {
    ...
    particleSys.emit = function(p) {
        ...
        var xPos = L + particleSys.particleRadius + (p - 1) * particleSys.particleRadius * 2.0;
    ...
    particleSys.move = function(l, p) {
        var yPos = (1.0 - l) * (B + particleSys.particleRadius + 1) + l * (T - particleSys.particleRadius);
        particles[p][POS][Y] = yPos;

        if (p == 0) {
            particles[p][POS][Y] = B + (yPos - B) / 2.0;
            initParticleField(p);
            return;
        }
...
function stoneParticles() {
    ...
    particleSys.particleRadius = (FIELD_HEIGHT_H / FIELD_HEIGHT) * 2.0;

    particleSys.emit = function(p) {
        % if (getRandomInt(100) > 1) {
        %     return false;
        % }
        %var xPos = (L + particleSys.particleRadius) + getRandomInt((FIELD_WIDTH_H - particleSys.particleRadius) * 2.0);
        const s = (L + particleSys.particleRadius);
        const w = (FIELD_WIDTH_H - particleSys.particleRadius) * 2.0;
        const pos = [
            [s + w * 0.25, T - particleSys.particleRadius],
            [s + w * 0.5, T],
            [s + w * 0.75, T - particleSys.particleRadius]
        ];
        const mom = [
            [-2, 2],
            [0, 2],
    ...
    particleSys.move = function(l, p) {
        % var yPos = (1.0 - l) * (T - particleSys.particleRadius) + l * (B + particleSys.particleRadius + 1);
        ...
        if (particles[p][POS][Y] <= B + particleSys.particleRadius && particles[p][SYMB] == 0) {
            particles[p][SYMB] = 1;
            particles[p][MASS] = 1;
            initParticleField(p); % TODO
        }
