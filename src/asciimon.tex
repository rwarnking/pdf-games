...
var particles = Array.apply(null, Array(ATK_ANIM_CELLS)).map(_ => [DEAD, [0, 0, 0, 0], 0, 0, 2]);
...
function applyGravityToParticle(p) {
    ...
    var position = [0, particles[p][POS][1] - SIZE_H];

    momentum = [momentum[X] + GRAVITY[X] + FORCE[X], momentum[Y] + GRAVITY[Y] + FORCE[Y]];
    velocity = [momentum[X] / particles[p][MASS], momentum[Y] / particles[p][MASS]];
    position = [position[X] + velocity[X], position[Y] + velocity[Y]];

    % If position is at floor & momentum heading downward
    if (position[Y] < B && momentum[Y] < 0) {
        position[Y] = B;
        % Bouncing off direction
        var collisionNormal = [0, 1];
        var dot = momentum[X] * collisionNormal[X] + momentum[Y] * collisionNormal[Y];
        % Reflect
        % momentum = momentum.subtr(collisionNormal.mu(2 * dot));
        momentum = [momentum[X] - collisionNormal[X] * 2 * dot, momentum[Y] - collisionNormal[Y] * 2 * dot];
        % Energy loss from bounce.
        const energyReduction = 0.45
        momentum = [momentum[X] * energyReduction, momentum[Y] * energyReduction];
    }
...
function attackAnimation(type) {
    if (type == T_STONE || true) {
        ...
        particleSys.emit = function(p) {
            ...
            %     return false;
            % }
            var xPos = (L + SIZE_H) + getRandomInt((FIELD_WIDTH_H - SIZE_H) * 2.0);
            var pos = [
                (L + SIZE_H) + (FIELD_WIDTH_H - SIZE_H) * 2.0 * 0.35,
                (L + SIZE_H) + (FIELD_WIDTH_H - SIZE_H) * 2.0 * 0.5,
                (L + SIZE_H) + (FIELD_WIDTH_H - SIZE_H) * 2.0 * 0.65
            ];

            var xPos = pos[p \% particleSys.particleCount];
            var xRect = [xPos - SIZE_H, T, xPos + SIZE_H, T];

            particles[p][STATE]++;
            particles[p][POS] = xRect;
        ...
        particleSys.move = function(l, p) {
            % var yPos = (1.0 - l) * (T - SIZE_H) + l * (B + SIZE_H + 1);
            % particles[p][POS][1] = yPos + SIZE_H;
            % particles[p][POS][3] = yPos - SIZE_H - 1;

            var pY = applyGravityToParticle(p);
            particles[p][POS][1] = pY + SIZE_H;
            particles[p][POS][3] = pY - SIZE_H - 1;
            ...
            if (particles[p][POS][3] <= B && particles[p][SYMB] == 0) {
                particles[p][SYMB] = 1;
                particles[p][MASS] = 1;
                initParticleField(p); % TODO
            }
        ...
        %     }
        % };
    } else if (type == T_WATER || true) {
        particleSys.iterationCount = (ATK_DURATION - 1) * 2;
        particleSys.particleLifetime = (ATK_DURATION - 1) * 4;
        particleSys.particleSymbols = ["(", " )", "(", "(", "(", " )", " )", "(", " )", " )"];%, "~"];
        particleSys.particleBold = true;
        particleSys.particleCount = FIELD_WIDTH / 2.0;

        particleSys.emit = function(p) {
            var xPos = L + SIZE_H + p * SIZE_H * 2;
            var xRect = [xPos - SIZE_H, 0, xPos + SIZE_H, 0];

            particles[p][STATE]++;
            particles[p][POS] = xRect;
            particles[p][SYMB] = p \% particleSys.particleSymbols.length;
            %if (p \% 3 != 2) {
        ...
        particleSys.move = function(l, p) {
            var yPos = (1.0 - l) * (B + SIZE_H + 1) + l * (T - SIZE_H);
            particles[p][POS][1] = yPos + SIZE_H;
            particles[p][POS][3] = yPos - SIZE_H - 1;

            particles[p][SYMB] = (particles[p][SYMB] + 1) \% particleSys.particleSymbols.length;
        ...
        particleSys.move = function(l, p) {
            const R = FIELD_WIDTH_H;
            const ANGLE = (Math.PI * 2.0) / particles.length;

            var offsetX = R * Math.cos(p * ANGLE + l * Math.PI * 2.0);
            var offsetY = R * Math.sin(p * ANGLE + l * Math.PI * 2.0);
            var offset = [offsetX, offsetY, offsetX, offsetY];

            var pos = [0, 0, 0, 0];
            for (var j = 0; j < pos.length; j++) {
                particles[p][POS][j] = END[j] + (1.0 - l) * offset[j];
            }
        }
    }

    attackInterval = app.setInterval("processParticles()", ATK_ANIMATION_SPEED);
}
