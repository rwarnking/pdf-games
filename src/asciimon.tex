...
var particles = Array.apply(null, Array(ATK_ANIM_CELLS)).map(_ => [
    ...
    (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 1.3
]);
...
function attackAnimation(type) {
    if (type == T_WATER && false) {
        waterParticles();
    } else if (type == T_AIR && false) {
        airParticles();
    } else if (type == T_STONE && false) {
        stoneParticles();
    } else if (type == T_GRASS || true) {
        grassParticles();
    } else if (type == T_GHOST && false) {
        ghostParticles();
    } else if (type == T_FLUFF || true) {
        fluffParticles();
    } else {
        % T_FIRE
        fireParticles();
...
function grassParticles() {
    ...
    particleSys.emit = function(p) {
        ...
        particles[p][POS][Y] = CENTER_P[Y] + offsetY;
        particles[p][SIZE] = (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 0.9;
        particles[p][ROTATION] = 90;
    ...
    particleSys.move = function(l, p) {
        particles[p][POS][X] = CENTER_P[X] * (1.0 - l) + CENTER_E[X] * l;
        particles[p][POS][Y] += ((CENTER_E[Y] - CENTER_P[Y]) / particleSys.particleLifetime);
    }
}
