...
% TODO bug when capturing creature (own creature moves even though it was not changed)
...
const ITEM_LIST = new Array(
    ...
    ["Drink", "Heals to full"],
    ["Weaken", "Deals a random amount of the enemy hp as dmg"],
    ...
    if (turnState == ACTION_END) {
        ...
        % Process gui input that was not in the main menu
            ...
            if (button == MENU) {
                ...
                var item = ITEM_LIST[curPlayerItems[button - 1]];
...
function executeRound(action, idx) {
    ...
    % Select what the enemy will do
        ...
        var attackData = ATTACK_LIST[lastAttackIdx];
        printInfoText(curEnemy[NAME] + " (Enemy) attacks with " + attackData[NAME] + "! ");
...
function attackEnemy(idx) {
    ...
    if (curEnemy[CUR_HP] == 0) {
        ...
        printInfoText(curEnemy[NAME] + " took " + dmg + " damage.");
    }
}
...
function attackPlayer(idx) {
    ...
    if (creature[CUR_HP] == 0) {
        ...
        printInfoText(creature[NAME] + " took " + dmg + " damage.");
    }
...
function itemApply(idx) {
    ...
    if (item[NAME] == "Empty") {
        ...
        var curCreature = curPlayerCreatures[curCreatureIdx];
        var attacks = curCreature[ATKS];
        var attackData = ATTACK_LIST[attacks[idx - 1]];
        % Set attack specific data
        attackAnimation(attackData[TYPE], ENEMY);
...
function itemSelect(idx) {
    var item = ITEM_LIST[curPlayerItems[selection - 1]];
...
function updateButtontxt() {
    if (guiState == MENU) {
        ...
        for (var i = 0; i < 4; i++) {
            var btn = "button" + (i + 1) + "txt";
            var name = ITEM_LIST[curPlayerItems[i]][NAME];
            if (name != "Empty") {
                name += " (" + ITEM_LIST[curPlayerItems[i]][DESC] + ")";
            }
            this.getField(btn).value = name;
        }
    }
...
function grassParticles(target) {
    ...
    particleSys.move = function(l, p) {
        ...
        var s = canvas.c[(target + 1) \% 2];
        var t = canvas.c[target];
        particles[p][POS][X] = s[X] * (1.0 - l) + t[X] * l;
        particles[p][POS][Y] += ((t[Y] - s[Y]) / particleSys.particleLifetime);
    }
}
...
function ghostParticles(target) {
    ...
    particleSys.move = function(l, p) {
        ...
        particles[p][POS][X] = canvas.c[target][X] + (1.0 - l) * offsetX;
        particles[p][POS][Y] = canvas.c[target][Y] + (1.0 - l) * offsetY;
    }
}
