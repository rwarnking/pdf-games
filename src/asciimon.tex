 ...
 */
function restart() {
    if (animationProgress != FIELD_WIDTH || particleSys.aliveParticles != 0) {
...
function attack(idx) {
    ...
    attackAnimation(attack[TYPE], PLAYER);
 ...
 */
function enemyTurn() {
    if (particleSys.aliveParticles != 0) {
        return;
    }
    ...
    % Start the attack animation with the player as target
    attackAnimation(attack[TYPE], ENEMY);
...
function defaultParticleSys() {
    particles = Array.apply(null, Array(ATK_ANIM_CELLS)).map(_ => [
        ...
        0, 0, % SYMB, ROTATION
        2, [0, 0],  % MASS, MOMENTUM
        FIELD_WIDTH_H / FIELD_HEIGHT, % WIDTH
        FIELD_HEIGHT_H / FIELD_HEIGHT, % HEIGHT
...
function attackAnimation(type, dir) {
    defaultParticleSys();
...
function waterParticles() {
    ...
    particleSys.move = function(l, p) {
        ...
        if (p == 0) {
            % Since the alignment of the brackeds is somewhat weird the box needs to be
...
function airParticles() {
    ...
    particleSys.emit = function(p) {
        ...
        if (p == leader) {
            ...
            yPos = B + particles[p][HEIGHT] + getRandomInt((FIELD_HEIGHT_H - particles[p][HEIGHT]) * 2.0);
            pos = [L + particles[p][WIDTH], yPos];
        } else {
            pos = [particles[p - 1][POS][X], particles[p - 1][POS][Y]];
            pos[X] -= 2.0 * particles[p][WIDTH];
            if (pos[X] < L + particles[p][WIDTH] || particles[p - 1][STATE] == DEAD) {
                return false;
            }
        }
    ...
    particleSys.move = function(l, p) {
        ...
        % This is a leader particle
        if (p == leader) {
            pos = applyGravityToParticle(p, B);
        } else {
            pos = [particles[p - 1][POS][X], particles[p][POS][Y]];
            % If the leader is already outofbounds
            if (pos[X] >= R - particles[p][WIDTH]) {
                pos[X] -= 2.0 * particles[p][WIDTH];
                % If the particle is near the leader
                if (particles[p][POS][X] == pos[X]) {
                    pos[X] = R - particles[p][WIDTH];
...
function stoneParticles() {
    ...
    % Falling stones move function
    particleSys.move = function(l, p) {
        particles[p][POS] = applyGravityToParticle(p, B);
...
function grassParticles(dir) {
    particleSys.iterationCount = (ATK_DURATION - 1) * 2;
    particleSys.particleSymbols = ["()", "#", "{}"];

    if (dir == ENEMY) {
        particleSys.source = CENTER_E;
        particleSys.target = CENTER_P;
    }
    ...
    particleSys.emit = function(p) {
        ...
        particles[p][POS][Y] = particleSys.source[Y] + offsetY;
        particles[p][SIZE] = (SCREEN_TEXT_WIDTH / FIELD_WIDTH) * 0.9;
        particles[p][ROTATION] = 90;
    ...
    particleSys.move = function(l, p) {
        var s = particleSys.source;
        var t = particleSys.target;
...
function fluffParticles() {
    ...
    particleSys.emit = function(p) {
        ...
        particles[p][POS] = [CENTER_P[X] - particles[p][WIDTH], B + particles[p][HEIGHT] + 10];
        particles[p][MOMENTUM] = [40, 15 * 6.0];
    ...
    particleSys.move = function(l, p) {
        ...
        if (particles[p][POS][X] >= CENTER_E[X] && particles[p][POS][Y] <= CENTER_E[Y]) {
    ...
    \begin{Form}
        ...
        \xintFor* #1 in {\xintSeq{0}{\atkAnimCells-1}} \do {%
            \TextField[%
                ...
                bordercolor=, backgroundcolor=, name=atkanim#1, readonly=true, borderwidth=0pt, hidden%
                %backgroundcolor=gray, name=atkanim#1, readonly=true, hidden, borderwidth=0pt, bordersep=0pt%
            ]{}%
        }%
