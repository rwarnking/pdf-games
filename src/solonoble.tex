...
function solve() {
    ...
    var movesToTry = getAllMoves();


    % Start backtracking until a solution is found
    % or all possibilities where tested.
    var solution = testSolution(movesToTry, currentMarbleCount);

    if (solution[0]) {
        printInfoText(diff1 + " Sol: " + solution);
    } else {
        printInfoText("No solution found! " + solution[1]);
    }
...
function testSolution(movesToTry, marbleCount) {
    % TODO safety check
    jasd++;
    if (jasd == 1000000) return [false, [999]];

    % If there is only one marble check for success
    if (marbleCount <= 1) {
        if (complField[W_POS][W_POS][0] == FULL) {
            return [true, []];
        } else {
            return [false, [111]];
        }
...
\begin{document}
    ...
    \begin{Form}
        ...
        \columnbreak
            \begin{tcolorbox}
                \TextField[name=importtxt, width=2\linewidth, bordercolor=, backgroundcolor=]{\strut}
            \end{tcolorbox}
        \columnbreak
        ...
        \columnbreak
            \begin{tcolorbox}
                \TextField[name=exporttxt, width=2\linewidth, bordercolor=, backgroundcolor=]{\strut}
            \end{tcolorbox}
        \end{multicols}
