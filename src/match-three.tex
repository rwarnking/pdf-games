...
function swap(x, y) {
    if (currentSlot !== null) {
        ...
        if (Math.abs(x - currentSlot[0]) + Math.abs(y - currentSlot[1]) == 1) {
            ...
            if (hasMatch(x, y) || hasMatch(currentSlot[0], currentSlot[1])) {
                printInfoText("Swapped stones!");
                % Redraw
                draw(x, y);
                draw(currentSlot[0], currentSlot[1]);
                % Start replacing stones
                replaceStones(x, y, currentSlot[0], currentSlot[1])
            % If it wasnt revert the swap
            } else {
                printInfoText("Swap not allowed!");
                % Swap contents back
                tmp = complField[currentSlot[1]][currentSlot[0]][0];
                complField[currentSlot[1]][currentSlot[0]][0] = complField[y][x][0];
                complField[y][x][0] = tmp;
            }
...
%%%%%%%%%%%%%%%%%%%%%%%%
/**
 * Calculates the direction counts for the given slot,
...
/**
 * Calculates the direction count for the given slot
