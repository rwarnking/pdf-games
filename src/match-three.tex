...
function restart() {
    ...
    printInfoText(endTime - startTime);
...
function swap(x, y) {
    if (currentCell !== null) {
        ...
        if (complField[y][x][2] == UNBREAKABLE || complField[y2][x2][2] == UNBREAKABLE) {
            printInfoText("Swap not allowed! Unmoveable stone.");
...
function dropAndAddStones(x) {
    ...
    for (var y = FIELD_HEIGHT - 1; y >= 0; y--) {
        ...
        if (complField[y][x][0] === EMPTY) {
            ...
            var y2 = y - 1;
            for (; y2 >= 0; y2--) {
                % If there is a non emtpy, removeable cell swap values
                if (complField[y2][x][0] !== EMPTY && complField[y2][x][2] !== UNBREAKABLE) {
...
function removeStones(x, y) {
    ...
    if (countXPos > 1) {
        ...
        for (var i = -complField[y][x][1][2]; i < complField[y][x][1][0] + 1; i++) {
            var tmpX = x + i;
            if (complField[y][tmpX][2] !== UNBREAKABLE) {
                complField[y][tmpX][0] = EMPTY;
                draw(tmpX, y);
                changedColumns[tmpX] = CHANGED;
            }
            points++;
        }
        stonesRemoved = true;
    ...
    if (countYPos > 1) {
        for (var i = -complField[y][x][1][3]; i < complField[y][x][1][1] + 1; i++) {
            var tmpY = y + i;
            if (complField[tmpY][x][2] !== UNBREAKABLE) {
                complField[tmpY][x][0] = EMPTY;
                draw(x, tmpY);
            }
            points++;
        }
