...
function restart() {
    ...
    for (var row = 0; row < BOARD_HEIGHT; row++) {
        for (var column = 0; column < BOARD_WIDTH; column++) {
            rnd = getRandomInt(cur_symbols.length);
            complBoard[row][column][0] = rnd;
            complBoard[row][column][2] = NORMAL;
        }
    }
...
function cleanupBoard() {
    ...
    for (var row = 0; row < BOARD_HEIGHT; row++) {
        for (var column = 0; column < BOARD_WIDTH; column++) {
            elem = complBoard[row][column][0];
            if (column > 1 &&
                complBoard[row][column - 1][0] == elem &&
                complBoard[row][column - 2][0] == elem) {

                complBoard[row][column][0] = getReplacementSymbol(row, column);
                %highlight(column, row, true);
            }
            if (row > 1 &&
                complBoard[row - 1][column][0] == elem &&
                complBoard[row - 2][column][0] == elem) {
...
function getReplacementSymbol(row, column) {
    ...
    if (column > 0) {
        neighbourhood[complBoard[row][column - 1][0]] = false;
    }
    if (column < BOARD_WIDTH - 1) {
        neighbourhood[complBoard[row][column + 1][0]] = false;
    }
    if (row > 0) {
        neighbourhood[complBoard[row - 1][column][0]] = false;
    }
    if (row < BOARD_HEIGHT - 1) {
        neighbourhood[complBoard[row + 1][column][0]] = false;
    }
    neighbourhood[complBoard[row][column][0]] = false;
...
function swap(x, y) {
    if (currentCell !== null) {
        ...
        var type = complBoard[y][x][2] | complBoard[y2][x2][2];
        if (type > NORMAL) {
            if ((type & BOMB) > 0) {
        ...
        if (Math.abs(x - x2) + Math.abs(y - y2) == 1) {
            % Swap contents
            var tmp = complBoard[y2][x2][0];
            complBoard[y2][x2][0] = complBoard[y][x][0];
            complBoard[y][x][0] = tmp;
...
function hasMatch(x, y) {
    ...
    var tmpDirCounts = complBoard[y][x][1];
    calcDirCounts(x, y);

    var countXPos = complBoard[y][x][1][0] + complBoard[y][x][1][2];
    var countYPos = complBoard[y][x][1][1] + complBoard[y][x][1][3];
...
function dropAndAddStones(x) {
    ...
    for (var y = BOARD_HEIGHT - 1; y >= 0; y--) {
        ...
        if (complBoard[y][x][0] === EMPTY && complBoard[y][x][2] !== OBSTACLE) {
            % Check all cells above the current cell
            var y2 = y - 1;
            for (; y2 >= 0; y2--) {
                % If there is a non emtpy, moveable cell swap values
                var type = complBoard[y2][x][2];
                if (complBoard[y2][x][0] !== EMPTY && type < UNBREAKABLE) {
                    complBoard[y][x][0] = complBoard[y2][x][0];
                    complBoard[y2][x][0] = EMPTY;
                    if (type !== NORMAL) {
                        complBoard[y][x][2] = type;
                        complBoard[y2][x][2] = NORMAL;
                    }
...
function clearPos(x, y) {
    ...
    if (complBoard[y][x][2] === BREAKTWICE) {
        complBoard[y][x][2] = NORMAL;
        draw(x, y);
    } else if (complBoard[y][x][2] < UNBREAKABLE) {
        complBoard[y][x][0] = EMPTY;
        draw(x, y);
...
function highlightStones(x, y, enable) {
    ...
    if (countXPos > 1) {
        ...
        for (var i = -complBoard[y][x][1][2]; i < complBoard[y][x][1][0] + 1; i++) {
            var tmpX = x + i;
    ...
    if (countYPos > 1) {
        for (var i = -complBoard[y][x][1][3]; i < complBoard[y][x][1][1] + 1; i++) {
            var tmpY = y + i;
...
function calcDirCounts(x, y) {
    complBoard[y][x][1][0] = calcDirCount(x, y, 1, 0);
    complBoard[y][x][1][1] = calcDirCount(x, y, 0, 1);
    complBoard[y][x][1][2] = calcDirCount(x, y, -1, 0);
    complBoard[y][x][1][3] = calcDirCount(x, y, 0, -1);
}
...
function calcDirCount(x, y, dirX, dirY) {
    ...
    while (nX >= 0 && nX < BOARD_WIDTH &&
        nY >= 0 && nY < BOARD_HEIGHT
    ) {
        % Check the symbol
        if (complBoard[nY][nX][0] === symb) {
            count++;
        } else {
            break;
        }
