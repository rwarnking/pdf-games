...
/**
 * Checks if a swap of the currentSlot and the given position
 * would leed to any matches with atleast 3 elements.
 * For this the slots are iterated and the symbol occurrences counted.
 *
 * @param {Number} x The x position.
 * @param {Number} y The y position.
...
function checkRemoval(x, y) {
    % Save the currentSlot for easier readability
    var x2 = currentSlot[0];
    var y2 = currentSlot[1];
    % There are two directions in which a match can occur, vertical and horizontal
    var dirs = [
        1, 0,
        0, 1
    ];
    ...
    for (var i = 0; i < dirs.length; i += 2) {
        % Get the symbols
        var symb1 = complField[y][x][0];
        var symb2 = complField[y2][x2][0];
        % The count is set to one because the tested slot does contribute one symbol
        var count1 = 1;
        var count2 = 1;

        % There need to be 3 succeeding symbols out of 5 positions
        for (var j = -2; j < 3; j++) {
            if (j == 0) {
                continue;
            }
            % Calc new position
            var nX = x2 + j * dirs[i];
            var nY = y2 + j * dirs[i + 1];
            % Check for out of bounds and the symbol itself
            % Test for x,y pos to cut one direction
            if (nX >= 0 && nX < FIELD_WIDTH &&
                nY >= 0 && nY < FIELD_HEIGHT &&
                complField[nY][nX][0] === symb1 && (nX != x || nY != y)
            ) {
                count1++;
            } else {
                % Reset count since the chain was broken
                count1 = 1;
            }
            ...
            if (nX >= 0 && nX < FIELD_WIDTH &&
                nY >= 0 && nY < FIELD_HEIGHT &&
                complField[nY][nX][0] === symb2 && (nX != x2 || nY != y2)
            ) {
                count2++;
            } else {
                % Reset count since the chain was broken
