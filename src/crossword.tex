...
function cellClicked(m, n) {
    printInfoText("clicked " + m + " " + n + " " + solution[m][n]);
    if (solution[m][n] >= "A") {
        complField[m][n] = solution[m][n];
    }
    draw(m, n);
}
...
function hideHint(m, n) {
    if (complField[m][n] > "Z") {
        ...
        printInfoText("hidehint ");
    }
}

function hideCell(m, n, state) {
    this.getField("cell" + m + "-" + n).hidden = state;
}
