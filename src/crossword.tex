...
% influence how many words can be placed onto the field.
var wordCount = 10;
...
% Globals
% The game- and the solution field
...
function initialiseButtons() {
    ...
    overlay.textSize = SCREEN_TEXT_WIDTH / Math.max(FIELD_HEIGHT, FIELD_WIDTH);
...
function restart() {
    ...
    curWords = [];
    % Create the field, by creating a solution and a list of current words
    createField();
...
%%%%%%%%%%%%%%%%%%
/**
 * Creates a field, by adding words from the word list to the solution array when possible.
...
function createField() {
    ...
    for (var i = 0; i < wordCount; i++) {
        var len = getRandomInt(Math.min(FIELD_WIDTH, FIELD_HEIGHT) - 1);
        lengthList.push(len);
    }
    % Sort length list
    ...
    for (var i = 0; i < wordCount; i++) {
        ...
        if (i == 0) {
            ...
            var n = Math.floor(FIELD_WIDTH * 0.5);
...
function addWord(word) {
    ...
    complField[m][n] = SYMBOLS[posInfo[ORI_IDX]];
...
function checkCollisionAndOOB(word, posInfo) {
    ...
    var tmpM = m - ori[M];
    var tmpN = n - ori[N];
    if (tmpN >= 0 && tmpN < FIELD_WIDTH && tmpM >= 0 & tmpM < FIELD_HEIGHT) {
...
%%%%%%%%%%%%%
/**
 * Assignes the current cell value to the complField array.
...
function cellClicked(m, n) {
    ...
    if (complField[m][n] > "Z") {
        cell.value = complField[m][n];
    } else {
        % Convert to uppercase
        if ("a" <= cell.value && cell.value <= "z") {
            cell.value = cell.value.toUpperCase();
            complField[m][n] = cell.value;
...
/**
 * Check the complete field for correct or incorrect inputs
...
function checkAll() {
    ...
    for (var w = 0; w < curWords.length; w++) {
        ...
        for (var c = 1; c < curWords[w].length; c++) {
            ...
            if (solution[m][n] != complField[m][n]) {
                wordComplete = false;
                gameover = false;
            }
        }
    ...
    \begin{Form}
            ...
            \begin{center}
                ...
                \xintFor* #1 in {\xintSeq{0}{\fieldHeight-1}} \do {%
                    \xintFor* #2 in {\xintSeq{0}{\fieldWidth-1}} \do {%
                        \TextField[%
                            name=cell#1-#2, bordercolor=gray,
        ...
        \begin{center}
            ...
            \begin{tabularx}{\textwidth}{@{} *{2}{X} @{}}%
                \begin{tcolorbox}
                    \TextField[name=info, width=\linewidth, readonly=true]{\strut}
                \end{tcolorbox} &
                \begin{tcolorbox}
                    \centering
                ...
                \end{tcolorbox}\\[-0.5cm]
                \PushButton[name=checkall, bordercolor=white, borderwidth=0]{
                    \begin{tcolorbox}
                        \centering
                        Check input\strut
                    \end{tcolorbox}
        ...
        \TextField[name=overlay, bordercolor=white, readonly, hidden]{}
