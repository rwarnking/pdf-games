...
function fillQuad(numberArray, offsetX, offsetY) {
    ...
    for (var idx = 0; idx < numberArray.length; idx++) {
        ...
        complBoard[y][x] = numberArray[idx];
    }
}
...
function solveSudoku() {
    ...
    % Apply candidates
    for (var idx = 0; idx < spaces.length; idx++) {
        complBoard[spaces[idx][1]][spaces[idx][0]] = candidates[idx][indices[idx]];

        % When the cell value is not valid increase indices and reset complBoard and idx
        if (!isCellValid(spaces[idx][0], spaces[idx][1])) {
            ...
            for (var idx2 = 0; idx2 <= idx; idx2++) {
                complBoard[spaces[idx2][1]][spaces[idx2][0]] = EMPTY;
            }
 ...
 *
 * @param {Number} valX X-coordinate of the cell
 * @param {Number} valY Y-coordinate of the cell
 * @return {Boolean} Returns true if cell value is valid else false.
 */
function isCellValid(valX, valY) {
    % Get the value
    var value = complBoard[valY][valX];

    % Check all possibly conflicting cells for the same value
    % Check the row
    for (var x = 0; x < BOARD_WIDTH; x++) {
        if (complBoard[valY][x] == value && x != valX) {
            return false;
        }
    }
    ...
    % Check the column
    for (var y = 0; y < BOARD_HEIGHT; y++) {
        if (complBoard[y][valX] == value && y != valY) {
            return false;
        }
    }
    ...
    for (var x = xQuad * BOX_WIDTH; x < xQuad * BOX_WIDTH + BOX_WIDTH; x++) {
        for (var y = yQuad * BOX_WIDTH; y < yQuad * BOX_WIDTH + BOX_WIDTH; y++) {
            if (complBoard[y][x] == value && (y != valY || x != valX)) {
                return false;
            }
        }
    }
...
function resetSudoku() {
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
            complBoard[y][x] = original[y][x];
            refreshCell(x, y, COLORS[1]);
        }
    }
}
...
function showSolution() {
    ...
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
            if (complBoard[y][x] == EMPTY || customMode) {
                complBoard[y][x] = solution[y][x];
                refreshCell(x, y, COLORS[0]);
            }
        }
    }
}
 ...
 */
function refreshCell(x, y, color) {
    var cell = this.getField("pos" + x + "-" + y);
    cell.delay = true;
    if (complBoard[y][x] > EMPTY) {
        cell.value = complBoard[y][x];
