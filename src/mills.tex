...
function checkMills(m, n) {
    ...
    if (n \% 2 == 0) {
        ...
        if (complBoard[m][n + 1] == p && complBoard[m][(n + 2) \% 8] == p ||
            complBoard[m][n_m2 + 1] == p && complBoard[m][n_m2] == p
        ) {
            return true;
        }
    } else {
        ...
        if (complBoard[m][n - 1] == p && complBoard[m][(n + 1) \% 8] == p ||
            complBoard[m_m1][n] == p && complBoard[m_p1][n] == p
        ) {
            return true;
        }
    }
...
function hasNoMoves(player) {
    ...
    for (var m = 0; m < ROWS; m++) {
        for (var n = 0; n < COLUMNS; n++) {
            ...
            for (var i = 0; i < NEIGHBOURS[m][n].length; i++) {
                var neighbour = NEIGHBOURS[m][n][i];
                if (complBoard[neighbour[0]][neighbour[1]] == EMPTY) {
                    return false;
                }
            }
        }
...
function highlight(m, n, enable) {
    var cell = this.getField("pos" + m + "-" + n);
    cell.delay = true;
    ...
    \begin{Form}
        ...
        \xintFor* #1 in {\xintSeq{0}{\rows-1}} \do {%
            \xintFor* #2 in {\xintSeq{0}{\columns-1}} \do {%
                \TextField[name=pos#1-#2, width=1.0cm, height=1.0cm,%
