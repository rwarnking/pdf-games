...
function initialise() {
    ...
    % Initialize the array with a cardindex
    ...
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
            ...
            for (var z = 0; z < SYMB_COUNT; z++) {
                var cell = this.getField("field" + z + x + y);
...
function restart() {
    ...
    for (var x = 0; x < BOARD_WIDTH; x++) {
        for (var y = 0; y < BOARD_HEIGHT; y++) {
            % Get random element from the cardList and remove the card afterwards
            var rnd = getRandomInt(unusedCards.length);
            complBoard[y][x] = unusedCards[rnd];
            unusedCards.splice(rnd, 1);
            draw(x, y);
        }
    }
...
function markCard(x, y) {
    ...
    if (isSet(lastSelections)) {
        ...
        for (var i = 0; i < lastSelections.length; i++) {
            ...
            complBoard[lastSelections[i][1]][lastSelections[i][0]] = -1;
        }
    ...
    for (var e1 = 0; e1 < cardCount - 2; e1++) {
        for (var e2 = e1 + 1; e2 < cardCount - 1; e2++) {
            for (var e3 = e2 + 1; e3 < cardCount; e3++) {
                % Reassign
                posArray[0] = [e1 \% BOARD_WIDTH, Math.floor(e1 / BOARD_WIDTH)];
                posArray[1] = [e2 \% BOARD_WIDTH, Math.floor(e2 / BOARD_WIDTH)];
                posArray[2] = [e3 \% BOARD_WIDTH, Math.floor(e3 / BOARD_WIDTH)];
...
function isSet(positions) {
    var idx1 = complBoard[positions[0][1]][positions[0][0]];
    var idx2 = complBoard[positions[1][1]][positions[1][0]];
    var idx3 = complBoard[positions[2][1]][positions[2][0]];
...
function refill() {
    ...
    if (slotCount == 0) {
        ...
        for (var x = 0; x < BOARD_WIDTH; x++) {
            for (var y = 0; y < BOARD_HEIGHT; y++) {
                unusedCards.push(complBoard[y][x]);
            }
        }
...
function draw(x, y) {
    ...
    for (var f = 0; f < SYMB_COUNT; f++) {
        var cell = this.getField("field" + f + x + y);
        cell.delay = true;
        cell.richText = true;
            ...
            \begin{tabularx}{\textwidth}{@{} *{\boardWidth}{X} @{}}%
                \xintFor* #2 in {\xintSeq{0}{\boardHeight-1}} \do {%
                    \xintFor* #1 in {\xintSeq{0}{\boardWidth-1}} \do {%
                        \PushButton[name=pos#1-#2, bordercolor=, backgroundcolor=, borderwidth=0]{%
                            \begin{tcolorbox}%
                                \centering%
                                % Sadly xintFor* does not work inside a PushButton
                                % so the elements need to be copy pasted
                                \TextField[name=field0#1#2, height=\cellHeight,%
                                    bordercolor=, backgroundcolor=, readonly]{}\\[2pt]%
                                \TextField[name=field1#1#2, height=\cellHeight,%
                                    bordercolor=, backgroundcolor=, readonly]{}\\[2pt]%
                                \TextField[name=field2#1#2, height=\cellHeight,%
                                    bordercolor=, backgroundcolor=, readonly]{}\\[2pt]%
                                \TextField[name=field3#1#2, height=\cellHeight,%
                                    bordercolor=, backgroundcolor=, readonly]{}\\[2pt]%
                                \TextField[name=field4#1#2, height=\cellHeight,%
