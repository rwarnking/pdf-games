 ...
 * Each move has two states, first being shown on the field and then being executed.
 * The the 0 element of the array is removed after the processing finishes.
 */
...
function convert(curTurn) {
    ...
    if (tgtM < 0 || tgtN < 0 || tgtM >= FIELD_HEIGHT || tgtN >= FIELD_WIDTH ||
        srcM >= FIELD_HEIGHT || srcN >= FIELD_WIDTH
    ) {
        return [];
    }
...
function getSourcePosition(tgtM, tgtN, srcM, srcN, piece) {
    for (var m = 0; m < FIELD_HEIGHT; m++) {
        ...
        for (var n = 0; n < FIELD_WIDTH; n++) {
            ...
            % Select the pieces with the currentTurn color
            if (currentTurn == complField[m][n][COLOR] && piece == complField[m][n][PIECE]) {
