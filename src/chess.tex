...
function highlightValidMoves(enable) {
    ...
    % King
        ...
        if (complField[lastSelection[M]][lastSelection[N]][MOVES] == 0) {
            for (var i = 0; i < rookPos[pColor + 1].length; i++) {
                ...
                if (complField[pos[M]][pos[N]][PIECE] == PIECE_SYMBOLS[ROOK] &&
                    ...
                    % TODO noCheck(m, n, pColor)
                    if (res) {
                        highlight(pos[M],  Math.round((pos[N] + lastSelection[N]) * 0.5),
                            POSS_MOVE_COLOR, enable);
                    }
                }
            }
        }
