...
% Parameter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Board width and height (should not be changed)
\def\boardWidth{8}
\def\boardHeight{8}
...
var promotionPos = [];
% If this is non -1 there is a pawn at these coords that might be taken via en passant (one turn)
var enpassant = [-1, -1];
...
% All input moves
var movesToProcess = [];
% The interval to keep applying moves
var playInterval;
% Save move state (show moves + the actual move)
var processBool = true;
% The move data after converting to internal representation
var conversion = [];
% Indicate whether there is a pawn to be promoted
var promotionIndex = 0;

% Game Data
% The symbol array, each piece gets one symbol.
% Since the utf8 chess pieces do not work default asccii letters are used. (0x2654 ...)
...
% Color data
const CELL_COLORS = new Array(
    [ "RGB", 0.8431, 0.6549, 0.3803 ],
    [ "RGB", 0.5686, 0.3058, 0.1098 ]
);

const TEXT_COLORS = new Array(
    color.white,
    color.black
);
...
function initialiseButtons() {
    ...
    for (var m = 0; m < BOARD_HEIGHT; m++) {
        for (var n = 0; n < BOARD_WIDTH; n++) {
            ...
            c.alignment = "center";
            c.textSize = SCREEN_TEXT_SIZE / (BOARD_WIDTH * 3);
            c.borderWidth = 4;
        }
    }
    ...
    var callback = "this.getField('process').setFocus();";
    this.getField("process").setAction("MouseDown", callback);
    this.getField("process").setAction("MouseUp", "processMoves();");
...
function restart() {
    ...
    if (movesToProcess.length != 0) {
        printInfoText("Can not restart! Wait for animation to finish!");
        return;
    }
    ...
    % Clear the board
    for (var m = 2; m < BOARD_HEIGHT - 2; m++) {
        for (var n = 0; n < BOARD_WIDTH; n++) {
             complBoard[m][n] = [EMPTY, NONE, UNMOVED];
        }
    }
...
function addPieces() {
    % Init PAWNs
    for (var n = 0; n < BOARD_WIDTH; n++) {
        complBoard[1][n] = [PIECE_SYMBOLS[PAWN], BLACK, UNMOVED];
        complBoard[BOARD_HEIGHT - 2][n] = [PIECE_SYMBOLS[PAWN], WHITE, UNMOVED];
    }

    % Init ROOKs, KNIGHTs, BISHOPs
    const PIECES = [PIECE_SYMBOLS[ROOK], PIECE_SYMBOLS[KNIGHT], PIECE_SYMBOLS[BISHOP]];
    for (var n = 0; n < PIECES.length; n++) {
        complBoard[0][n] = [PIECES[n], BLACK, UNMOVED];
        complBoard[0][BOARD_WIDTH - n - 1] = [PIECES[n], BLACK, UNMOVED];
        complBoard[BOARD_HEIGHT - 1][n] = [PIECES[n], WHITE, UNMOVED];
        complBoard[BOARD_HEIGHT - 1][BOARD_WIDTH - n - 1] = [PIECES[n], WHITE, UNMOVED];
    }

    % Init KINGs and QUEENs
    var centerpos = Math.floor(BOARD_WIDTH / 2);
    complBoard[0][centerpos - 1] = [PIECE_SYMBOLS[QUEEN], BLACK, UNMOVED];
    complBoard[0][centerpos] = [PIECE_SYMBOLS[KING], BLACK, UNMOVED];
    complBoard[BOARD_HEIGHT - 1][centerpos - 1] = [PIECE_SYMBOLS[QUEEN], WHITE, UNMOVED];
    complBoard[BOARD_HEIGHT - 1][centerpos] = [PIECE_SYMBOLS[KING], WHITE, UNMOVED];
}
 ...
 * 3. If the player selects the same cell twice the highlight is reversed
...
function move(m, n) {
    ...
    % If a selection is already saved check if the move is valid and if so move the piece there
        ...
        % Move piece
        complBoard[m][n] = complBoard[lastSelection[M]][lastSelection[N]];
        % The piece was moved, mark it as such
        complBoard[m][n][MOVES] = complBoard[lastSelection[M]][lastSelection[N]][MOVES] + 1;
        complBoard[lastSelection[M]][lastSelection[N]] = [EMPTY, NONE, UNMOVED];
        % Redraw
        draw(m, n);
        draw(lastSelection[M], lastSelection[N]);

        if (complBoard[m][n][PIECE] == PIECE_SYMBOLS[KING]) {
            ...
            if (Math.abs(lastSelection[N] - n) == 2) {
                ...
                complBoard[m][newN] = complBoard[m][rookN];
                % The rook was moved the first time
                complBoard[m][newN][MOVES] = 1;
                complBoard[m][rookN] = [EMPTY, NONE, UNMOVED];
                draw(m, newN);
                draw(m, rookN);
            }
        }

        if (complBoard[m][n][PIECE] == PIECE_SYMBOLS[PAWN]) {
            % Special case: en passant
            % Delete the pawn that was captured by en passant
            if (m == (enpassant[M] + currentTurn) && n == enpassant[N]) {
                complBoard[enpassant[M]][enpassant[N]] = [EMPTY, NONE, UNMOVED];
    ...
    % If the input matches the already selected cell, remove selection
        ...
        if (check) {
            highlight(
                kingPos[currentTurn + 1][M], kingPos[currentTurn + 1][N], CHECK_COLOR, check
            );
        }
    }
}
...
function validSelection(m, n) {
    % Empty cell
    if (complBoard[m][n][PIECE] == EMPTY) {
        printInfoText("Field empty, could not select!");
        return false;
    }
    % Enemy piece
    else if (currentTurn != complBoard[m][n][COLOR]) {
...
/**
 * To check if a move is valid the bordercolor of the target cell that was selected is
...
function validMove(m, n) {
    ...
    if (currentTurn == complBoard[m][n][COLOR]) {
    ...
    if (equal(cell.borderColor, POSS_MOVE_COLOR)) {
        ...
        printInfoText(
            complBoard[lastSelection[M]][lastSelection[N]][PIECE] + " can not move there."
        );
        return false;
    }
...
function highlightValidMoves(enable) {
    % Pawn
    if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[PAWN]) {
        var pieceColor = complBoard[lastSelection[M]][lastSelection[N]][COLOR];
        % Was the pawn already moved?
        const startPos = complBoard[lastSelection[M]][lastSelection[N]][MOVES] == UNMOVED;
        ...
        const positions = [
            ...
            [pieceColor, 1, diffCol],
            % Allow moving forward when the cell is empty
        ...
        for (var i = 0; i < positions.length; i++) {
            ...
            % Out of bounds check
            if (m < 0 || n < 0 || m >= BOARD_HEIGHT || n >= BOARD_WIDTH) {
                continue;
            }
            var targetColor = complBoard[m][n][COLOR];
            % Test if the jumped cell is free in case of a two cell movement
            if (Math.abs(positions[i][M]) == 2) {
                targetColor += complBoard[lastSelection[M] + pieceColor][n][COLOR];
            }
    ...
    % Queen
    else if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[QUEEN]) {
        loopPositions(DIRS[QUEEN], Math.max(BOARD_WIDTH, BOARD_HEIGHT), enable);
    }
    % Knight
    else if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[KNIGHT]) {
        loopPositions(DIRS[KNIGHT], 2, enable);
    }
    % Rook
    else if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[ROOK]) {
        loopPositions(DIRS[ROOK], Math.max(BOARD_WIDTH, BOARD_HEIGHT), enable);
    }
    % Bishop
    else if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[BISHOP]) {
        loopPositions(DIRS[BISHOP], Math.max(BOARD_WIDTH, BOARD_HEIGHT), enable);
    }
    % King
    else if (complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[KING]) {
        ...
        for (var i = 0; i < DIRS[KING].length; i++) {
            ...
            % Out of bounds
            if (m < 0 || n < 0 || m >= BOARD_HEIGHT || n >= BOARD_WIDTH ||
                pieceColor == complBoard[m][n][COLOR]
            ) {
                continue;
            }

            % Unless there is an enemy king in the neighborhood of that cell
            var result = true;
            for (var m2 = m - 1; m2 < m + 2; m2++) {
                for (var n2 = n - 1; n2 < n + 2; n2++) {
                    % Out of bounds check
                    if (m2 < 0 || n2 < 0 || m2 >= BOARD_HEIGHT || n2 >= BOARD_WIDTH) {
                        continue;
                    }
                    % Check for enemy king
                    if (complBoard[m2][n2][PIECE] == PIECE_SYMBOLS[KING] &&
                        complBoard[m2][n2][COLOR] != pieceColor
                    ) {
                         result = false;
                    }
                }
        ...
        % Special case: castling
        var pColor = complBoard[lastSelection[M]][lastSelection[N]][COLOR];
        var rookPos = [
            [[7, 0, +1], [7, 7, -1]], % white
            [[], []], % padding
            [[0, 0, +1], [0, 7, -1]] % black
        ...
        % for castling
        if (complBoard[lastSelection[M]][lastSelection[N]][MOVES] == 0 && !check) {
            ...
            for (var i = 0; i < rookPos[pColor + 1].length; i++) {
                ...
                if (complBoard[pos[M]][pos[N]][PIECE] == PIECE_SYMBOLS[ROOK] &&
                    complBoard[pos[M]][pos[N]][MOVES] == 0
                ) {
                    var res = true;
                    ...
                    for (var n = pos[N] + pos[2]; n != lastSelection[N]; n += pos[2]) {
                        res &= complBoard[pos[M]][n][PIECE] == EMPTY;
                    }
                    % Check if cells are in check
...
/**
 * Loops over the cells in all given directions for the specified number of steps.
...
function testCheck(col) {
    ...
    % in that case check whether it would be able to capture the king
    for (var i = 0; i < DIRS[KING].length; i++) {
        % Iterate as far as possible
        for (var o = 1; o < Math.max(BOARD_WIDTH, BOARD_HEIGHT); o++) {
            ...
            if (complBoard[m][n][PIECE] != EMPTY) {
                ...
                if (complBoard[m][n][PIECE] == PIECE_SYMBOLS[QUEEN] ||
                    (complBoard[m][n][PIECE] == PIECE_SYMBOLS[BISHOP] && (i \% 2 == 1)) ||
                    (complBoard[m][n][PIECE] == PIECE_SYMBOLS[ROOK] && (i \% 2 == 0)) ||
                    (complBoard[m][n][PIECE] == PIECE_SYMBOLS[PAWN] &&
    ...
    for (var i = 0; i < DIRS[KNIGHT].length; i++) {
        ...
        if (complBoard[m][n][PIECE] == PIECE_SYMBOLS[KNIGHT]) {
            return true;
        }
    }
...
function noCheck(m, n, color) {
    ...
    var isKing = complBoard[lastSelection[M]][lastSelection[N]][PIECE] == PIECE_SYMBOLS[KING];
    if (isKing) {
        kingPos[color + 1] = [m, n];
    }

    % Set the piece to the given position
    var tmp = complBoard[m][n];
    complBoard[m][n] = complBoard[lastSelection[M]][lastSelection[N]];
    complBoard[lastSelection[M]][lastSelection[N]] = [EMPTY, NONE, UNMOVED];
    ...
    complBoard[lastSelection[M]][lastSelection[N]] = complBoard[m][n];
    complBoard[m][n] = tmp;
...
function promotePawn(sel) {
    ...
    if (promotionPos.length > 0) {
        ...
        complBoard[promotionPos[M]][promotionPos[N]][PIECE] = PIECE_SYMBOLS[QUEEN + sel];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Process positional input %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/**
 * Process all moves that are given via the moves-field.
 * These moves should be of PGN format.
 * Multiple PGN examples where used to validate the correctness of
 * the function. These can be found here:
 * https://en.wikipedia.org/wiki/Portable_Game_Notation
 */
function processMoves() {
    this.getField("info").setFocus();
    % Prevent applying input to a finished game
    if (checkmate || stalemate) {
        printInfoText("Game over, please restart the game!");
        return;
    }

    % Get the input string
    var input = this.getField("moves").value;
    % Remove comments from input
    input = input.replace(/ *\{[^\}]*\}*/g, "");
    % Split the turns using the turn-number
    var allTurns = input.split(/\s*\d+\.\s*|,\s*/);

    % Remove empty string when splitting numbered list
    if (input.indexOf('.') > -1) {
        allTurns.splice(0, 1);
    }

    % Add both parts of a turn to the moves array
    movesToProcess = [];
    for (var idx = 0; idx < allTurns.length; idx++) {
        var parts = allTurns[idx].split(" ");
        for (var j = 0; j < parts.length; j++) {
            movesToProcess.push(parts[j]);
        }
    }

    printInfoText("Start interval and move processing.");
    playInterval = app.setInterval("processNextMove();", PLAYBACK_SPEED);
}

/**
 * Processes the move at position 0 from the movesToProcess array until the array is empty.
 * Each move has two states, first being shown on the board and then being executed.
 * The 0 element of the array is removed after the processing finishes.
 */
function processNextMove() {
    if (movesToProcess.length == 0) {
        printInfoText("All moves were processed.");
        app.clearInterval(playInterval);
        return;
    }

    % Test whether the move should be applied or shown
    if (processBool) {
        % Convert the move to a src and tgt position and save it for the next step
        conversion = convert(movesToProcess[0]);
        % If the conversion does not have a src and a tgt position, either
        % the game is over or the move is faulty
        if (conversion.length != 2) {
            if (conversion.length == 1) {
                printInfoText("Game over, " + conversion[0] + " won.");
            } else {
                printInfoText("Could not find a valid move for " + movesToProcess[0] + ".");
            }
            % Remove unprocessed elements
            movesToProcess.length = 0
            app.clearInterval(playInterval);
            return;
        }
        move(conversion[0][M], conversion[0][N]);
    } else {
        move(conversion[1][M], conversion[1][N]);
        % Test for promotion
        if (promotionPos.length > 0) {
            promotePawn(promotionIndex);
        }
        % Remove the processed move
        movesToProcess.splice(0, 1);
    }

    printInfoText("Moving: " + conversion[0] + " to " + conversion[1] +
        ". Remaining: " + movesToProcess);
    processBool = !processBool;
}

/**
 * This function is able to parse PGN notation.
 * These inputs can be processed:
 * - castles O-O and O-O-O
 * - pawn movements -> cb5
 * - other piece movements -> Nbd7
 * - pawn promotion -> e8=Q#
 * - result info -> 1/2-1/2, 0-1, 1-0
 * These move information are ignored by removing the symbols:
 * - captures -> x
 * - check -> +
 * - checkmate -> #
 * - moveinfo -> ! and ?
 *
 * Using the PGN notation the input is converted into the local coordinate system
 * with m and n coordinates for the target and the source.
 *
 * @param {Array} curTurn A single turn of the input that shall be converted.
 * @return {Array} The converted position in form of src and tgt position.
 */
function convert(curTurn) {
    var result = [];
    var piece = "P";
    ...
    % Delete all additional symbols that are not necessary to move the piece
    var turnClean = curTurn.replace(/x|\+|#|!|\?/g, "");
    ...
    % Game over, black won
    else if (turnClean == "0-1") {
        checkmate = true;
        return ["Black"];
    }
    % Game over, noone won
    else if (turnClean == "1\/2-1\/2") {
        stalemate = true;
        return ["No one"];
    }
    % King side castles
    else if (turnClean == "O-O") {
        tgtM = currentTurn == WHITE ? 7 : 0;
        tgtN = 6;
        piece = "K";
    }
    % Queen side castles
    else if (turnClean == "O-O-O") {
        tgtM = currentTurn == WHITE ? 7 : 0;
        tgtN = 2;
        piece = "K";
    } else {
        var characters = turnClean.split("");
        % Get the piece that should be moved
        if (characters.length == 2) {
            % piece = "P";
        } else if (characters[0].charCodeAt(0) - 90 > 0) {
            % piece = "P";
            % Pawn promotion
            if (characters[characters.length - 2] == "=") {
                var promotionChar = characters[characters.length - 1];
                if (promotionChar == "Q") {
                    promotionIndex = 0;
                ...
                characters.splice(characters.length - 2);
            }
        } else {
            piece = characters[0];
        ...
        var tgtM = 8 - parseInt(characters[characters.length - 1], 10);
        var tgtN = characters[characters.length - 2].charCodeAt(0) - 97;

        % Check if there are additional movement information
        for (var charPos = 0; charPos <= characters.length - 3; charPos++) {
            var column = characters[charPos].charCodeAt(0) - 97;
            if (column >= 0) {
                srcN = column;
            } else {
                srcM = 8 - parseInt(characters[charPos], 10);
            }
        }
    }

    % Only allow letters a-h and only numbers from 1-8
    if (tgtM < 0 || tgtN < 0 || tgtM >= BOARD_HEIGHT || tgtN >= BOARD_WIDTH ||
        srcM >= BOARD_HEIGHT || srcN >= BOARD_WIDTH
    ) {
        return [];
    }

    % After validating the target position the source position must be obtained
    var srcPos = getSourcePosition(tgtM, tgtN, srcM, srcN, piece);
    if (srcPos.length != 2) {
        return [];
    } else {
        % Return the source and target positions
        result.push(srcPos);
        result.push([tgtM, tgtN]);
    }
...
/**
 * Iterates all cells and checks whether the cell-piece matches the piece that is given
 * and has the same color as the currentTurn.
 * Each time a valid piece is found its moves are highlighted and validMove is called to
 * validate if the move is possible. If the move is possible the piece position is returned
 * to be used as source position. All highlights are disabled after each test.
 * Should it not be possible to find a matching piece an empty array is returned.
 * Additionally the source row or column can be specified to limit the search radius.
 *
 * @param {Number} tgtM The target row index.
 * @param {Number} tgtN The target column index.
 * @param {Number} srcM The source row index, use 0 is no row is known.
 * @param {Number} srcN The source column index, use 0 is no row is known.
 * @param {String} piece The search piece character.
 */
function getSourcePosition(tgtM, tgtN, srcM, srcN, piece) {
    for (var m = 0; m < BOARD_HEIGHT; m++) {
        % If a row is given only check this one row
        if (srcM > -1 && m != srcM) {
            continue;
        }

        for (var n = 0; n < BOARD_WIDTH; n++) {
            % If a column is given only check this one column
            if (srcN > -1 && n != srcN) {
                continue;
            }

            % Select the pieces with the currentTurn color
            if (currentTurn == complBoard[m][n][COLOR] && piece == complBoard[m][n][PIECE]) {

                lastSelection = [m, n];
                % Mark all cells that are accessible
                highlightValidMoves(true);

                if (validMove(tgtM, tgtN)) {
                    % Disable highlight
                    highlightValidMoves(false);
                    lastSelection = [];
                    return [m, n];
...
function draw(m, n) {
    ...
    cell.value = complBoard[m][n][PIECE];

    % Adjust piece color dependent on its owner
    if (complBoard[m][n][COLOR] == WHITE) {
        cell.textColor = TEXT_COLORS[WHITE + 1];
    } else if (complBoard[m][n][COLOR] == BLACK) {
        cell.textColor = TEXT_COLORS[BLACK];
    }

    % Adjust bordcolor
    if ((m + n) \% 2 == 0) {
        cell.borderColor = CELL_COLORS[WHITE + 1];
        cell.fillColor = CELL_COLORS[WHITE + 1];
    } else {
        cell.borderColor = CELL_COLORS[BLACK];
        cell.fillColor = CELL_COLORS[BLACK];
    }
            ...
            \begin{center}
                ...
                \xintFor* #1 in {\xintSeq{0}{\boardHeight-1}} \do {%
                    \xintFor* #2 in {\xintSeq{0}{\boardWidth-1}} \do {%
                        \TextField[%
                            name=cell#1-#2, bordercolor=white,%
        ...
        \begin{center}%
            ...
            \begin{tabularx}{\textwidth}{@{} *{4}{X} @{}}%
                \xintFor* #1 in {\xintSeq{0}{3}} \do {%
                    \begin{tcolorbox}
                        \TextField[
                            name=promote#1, width=\linewidth, bordercolor=gray, readonly
                        ]{}
                    \end{tcolorbox}
            ...
            \begin{tabularx}{\textwidth}{@{} *{2}{X} @{}}%
                \begin{tcolorbox}
                    \TextField[name=moves, bordercolor=gray, width=\linewidth]{}
                \end{tcolorbox} &%
                \PushButton[name=process, bordercolor=white]{
                    \begin{tcolorbox}
                        \centering
                        Process moves
                        \strut
