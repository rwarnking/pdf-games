...
% - captures -> replace all x in the move since it is not parsed anyway
% - check and checkmate -> replace all + and # -> Bxf7+
% - additional positioning information -> Nbd7
% - pawn is identified by adding no piece info -> cxb5
% - pawn promotion -> e8=Q#
% - result info -> 1/2-1/2
...
function convert(curTurn) {
    ...
    if (curTurn2.length == 3) {
        if (curTurn2[2] != " ") {
            % TODO check if the char is a piece char
            % Else use it for m or n
            if (curTurn2.charCodeAt(charPos) < 90) {
                piece = curTurn2.charAt(charPos++);
            } else {
                charPos++;
            }
        }
    }
    % len = 2 means a pawn needs to move
    else if (curTurn2.length == 2) {
        piece = "P";
    }

    % OBACHT: order is important
    var tgtN = curTurn2.charCodeAt(charPos++) - 97;
    var tgtM = 8 - parseInt(curTurn2.charAt(charPos++), 10);
    ...
    var todasd = getSourcePosition(tgtM, tgtN, piece);
    if (todasd.length != 2) {
        return [];
    } else {
        result.push(todasd);
...
function getSourcePosition(tgtM, tgtN, piecesa=0) {
    for (var m = 0; m < FIELD_HEIGHT; m++) {
        for (var n = 0; n < FIELD_WIDTH; n++) {
            % Select the pieces with the currentTurn color

            if (currentTurn == complField[m][n][COLOR] &&
                (piecesa == 0 || piecesa == complField[m][n][PIECE])
            ) {
