...
const PER_CHAR = CANVAS_WIDTH / FIELD_WIDTH;
% Calculate units per line
const PER_LINE = CANVAS_HEIGHT / FIELD_HEIGHT;
% Gravity stats
...
function initialiseButtons() {
    ...
    for (var m = 0; m < FIELD_HEIGHT; m++) {
        ...
        line.textSize = SCREEN_TEXT_WIDTH / FIELD_WIDTH;
 ...
 * It handels all game events like input, movement, spawning and
 * game-field adjustments. Each iteration is similar to one frame.
 */
function gameLoop() {
    % Cancel the interval
    if (gameover) {
    ...
    % Player movement, starts after the game-field is generated
    if (fieldCounter >= FIELD_WIDTH) {
        processInput();
    ...
    % Starts after the game-field is generated
    if (fieldCounter >= FIELD_WIDTH) {
        % Refresh obstacles
        refreshObstacles();
        % Draw the obstacles
...
function refreshObstacles() {
    ...
    for (var i = 0; i < OBSTACLE_COUNT; i++) {
        ...
        if (obstacles[i][X] < CANVAS_LTRB[LEFT] && spawn) {
            ...
            var lBound = CANVAS_LTRB[BOT] + (FIELD_HEIGHT - curBot) * PER_LINE + OBSTACLE_WIDTH;
            ...
            % Move obstacle such that the speed matches the movement of the game-field
...
%
/**
 * Checks the current player hitbox against the top and bottom line.
...
function testCollisionLine() {
    ...
    % Approximate the current char postion at the center of the player
    ...
    var curTop = getLine(fieldCounter - FIELD_WIDTH + sample, START_TOP_LINE);
    var curBot = getLine(fieldCounter - FIELD_WIDTH + sample, START_BOT_LINE);
...
function drawField() {
    ...
    for (var m = 0; m < FIELD_HEIGHT; m++) {
        ...
        % Shift only after the field was filled once
        if (fieldCounter >= FIELD_WIDTH) {
 ...
 * border of the canvas. Sets also the value of the field to the obstacle symbol.
 */
    ...
    \begin{Form}
        ...
        \begin{center}
            \xintFor* #1 in {\xintSeq{0}{\fieldHeight-1}} \do {%
                \TextField[%
                    name=line#1, bordercolor=,%
                    width=\blocksizeW, height=\blocksizeH, readonly%
                ]{}%
                \\[0pt]%
            }%
        \end{center}
        ...
        \begin{center}
            ...
            \begin{tabularx}{\textwidth}{@{} *{2}{X} @{}}%
                ...
                \begin{tcolorbox}
                    \TextField[name=info, width=\linewidth, readonly]{}
                \end{tcolorbox}
            \end{tabularx}
        \end{center}
